<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="myOrderApproval.ascx.cs" Inherits="StarsProject.myOrderApproval" %>
    ...
    <asp:Repeater ID="rptApproval" runat="server" OnItemDataBound="rptApproval_ItemDataBound">
        <HeaderTemplate>
            <thead>
                <tr>
                    ...
                    <th class="left-align">Sales Person</th>
                    <%--<th class="center-align">Issued By</th>--%>
        ...
        <ItemTemplate>
            <tr class="blueShed">
                ...
                <asp:HiddenField ID="hdnCustomerName" runat ="server" ClientIDMode="Static" Value='<%# (Eval("IssuedBy").ToString() == "Dealer") ? Eval("DealerName") : Eval("CustomerName") %>' />
                <td class="center-align">
                    <a href="javascript:viewSalesOrder(<%# Eval("pkID")%>);"><%# Eval("OrderNo") %></a>
                </td>
                <td class="center-align"><%# Eval("OrderDate", "{0:dd/MM/yyyy}") %></td>
                <td class="left-align"><%# (Eval("IssuedBy").ToString() == "Dealer") ? Eval("DealerName") : Eval("CustomerName") %></td>
                <td class="left-align"><%# (Eval("IssuedBy").ToString() == "Dealer") ? Eval("DealerName") : Eval("EmployeeName") %></td>
