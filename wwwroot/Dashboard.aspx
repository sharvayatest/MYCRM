<%@ Page Title="" Language="C#" MasterPageFile="~/StarsSite.Master" AutoEventWireup="true" CodeBehind="Dashboard.aspx.cs" Async="true" Inherits="StarsProject.Dashboard" MaintainScrollPositionOnPostback="true" %>
...
<%@ Register Src="~/myPendingMedicalLead.ascx" TagPrefix="uc1" TagName="myPendingMedicalLead" %>
...
<%@ Register Src="~/MyInquiryStatus.ascx" TagPrefix="uc1" TagName="MyInquiryStatus" %>
<%--<%@ Register Src="~/myDashboardInquirySummary.ascx" TagPrefix="uc1" TagName="myDashboardInquirySummary" %>
<%@ Register Src="~/myDashboardInquiryClinicSummary.ascx" TagPrefix="uc1" TagName="myDashboardInquiryClinicSummary" %>--%>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">  
    ...
    <script type="text/javascript">
        ...
        var togl = 1;
        // *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        // Loading Inquiry Graphical Analysis
        // *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        ...
        // *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        // Loading Inquiry Clinic Graphical Analysis
        // *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        function initClinicGraphLayout(cat) {
            var grLegend = '';
            var m1, y1;
            m1 = jQuery("#drpSummaryMonth").val();
            y1 = jQuery("#drpSummaryYear").val();
            ...
            // ----------------------------------------------------------
            var srcUrl = "";
            srcUrl = 'Services/NagrikService.asmx/GetDashboard' + cat + 'ClinicSummary';
            if (cat == 'Inquiry') {
                grLegend = 'Count';
                jQuery("#dash_InqStatus").show("slide");
            }
            ...
            // -----------------------------------------------------------
            if (srcUrl != '' && srcUrl != null) {
                jQuery("#linechartclinic").empty();
                jQuery("#donutchart1clinic").empty();
                jQuery("#donutchart2clinic").empty();
                // ---------------------------------------
                // Loading Line Graph
                // ---------------------------------------
                jQuery.ajax({
                    ...
                    success: function (data) {
                        ...
                        // ---------------------------------------
                        if (resultdata.length == 0 || data.d == undefined) {
                            jQuery("#linechartclinic").parent().attr("class", "hide");
                            jQuery("#label").hide();
                            var resultdata = [{ label: "No Data Availble", value: 0 }];
                        }
                        else {
                            jQuery("#linechartclinic").parent().attr("class", "show");
                        }
                        // ---------------------------------------
                        var linechartinq;
                        ...
                        if (resultdata.length > 0) {
                            if (cat == 'Inquiry') {
                                linechartinq = new Morris.Bar({
                                    element: 'linechartclinic', resize: true, data: resultdata,
                                    xkey: 'label', ykeys: ['value1', 'value2', 'value3', 'value4', 'value5', 'value6'],
                                    count: [1, 2, 1, 2, 1, 5],
                                    labels: ['Open', 'WIP', 'Hold', 'Lost', 'Success', 'Other'],
                                    barColors: ['#007bff', '#17a2b8', '#6c757d', '#c82333', '#28a745', '#ffc107'],
                        ...
                        jQuery.ajax({
                            type: "POST", url: srcUrl,
                            data: '{pMon: ' + m1 + ', pYear: ' + y1 + ', pCategory:\'CITY\'}',
                            contentType: "application/json; charset=utf-8",
                            success: function (data) {
                                var resultdata1 = JSON.parse(data.d);
                                if (resultdata1.length > 0) {
                                    var donutchart1inq = new Morris.Donut({
                                        element: 'donutchart1clinic', resize: true,
                                        //colors: ['#007bff', '#dc3545', '#28a745', '#17a2b8', '#ffc107', '#000080', '#6c757d', '#ff6a52', '#8afbe6'],
                                        data: resultdata1, hideHover: 'auto'
                                    })
                                    // ---------------------------------------
                                    donutchart1inq.redraw();
                                ...
                                jQuery.ajax({
                                    type: "POST", url: srcUrl,
                                    data: '{pMon: ' + m1 + ', pYear: ' + y1 + ', pCategory:\'SPECIALITY\'}',
                                    contentType: "application/json; charset=utf-8",
                                    success: function (data) {
                                        var resultdata2 = JSON.parse(data.d);
                                        if (resultdata2.length>0) {
                                            var donutchart2inq = new Morris.Donut({
                                                element: 'donutchart2clinic', resize: true,
                                                //colors: ['#007bff', '#dc3545', '#28a745', '#17a2b8', '#ffc107', '#000080', '#6c757d', '#ff6a52', '#8afbe6'],
                                                data: resultdata2, hideHover: 'auto'
                                            })
                                            // ---------------------------------------
                ...
                jQuery.ajax({
                    ...
                    success: function (data) {
                        ...
                        extchart = new Morris.Bar({
                            ...
                            xkey: 'label', ykeys: ['value1', 'value2', 'value3'], count: [1, 2, 1, 2, 1, 5],
                            labels: ['Qualified', 'DisQualified', 'Untouch'],
        ...
        function openCustomerView(t11) {
            ...
            if (clienturl.indexOf("femicure") >= 0) {
                //  This section is for FEMICURE Inquiry Additional column handling
                $.colorbox({
                    width: "98%", height: "98%", rel: 'cbInq', inline: true, iframe: true, href: "myCustomView.aspx?view=inquiryclinic&para1=" + t11 + '&para2=' + t12 + '&para3=' + t13,
                    onClosed: function () { }
                });
            }
            else {
        ...
        function openTimeline(xView, xCustID,xCustName) {
            $.colorbox({ fixed: true, width: "80%", height: "98%", rel: 'cbTimeline', iframe: true, href: "FollowupTimeline.aspx?view=" + xView + "&id=" + xCustID + "&name=" + xCustName });
        }
        // ------------------------------------------------------------------------
        ...
        function openDashboardBox(view, pID) {
            if (view=='todo') {
                var flagOpt, todoPageUrl;
                flagOpt = document.getElementById("hdnTODOVersion").value;
                todoPageUrl = (flagOpt == "1" || flagOpt == '') ? "ToDo.aspx?id=0" : "TODOMultiple.aspx?mode=add&id=0";
                (flagOpt == "1" || flagOpt == '') ? $.colorbox({ width: "95%", height: "85%", iframe: true, href: todoPageUrl, onClosed: function () { location.reload(); } }) : $.colorbox({ width: "95%", height: "85%", iframe: true, href: todoPageUrl, onClosed: function () { location.reload(); } });
            }
            if (view == 'todoview') {
                var flagOpt, todoPageUrl;
                flagOpt = document.getElementById("hdnTODOVersion").value;
                todoPageUrl = (flagOpt == "1" || flagOpt == '') ? "ToDo.aspx?mode=view&id=" + pID : "TODOMultiple.aspx?mode=view&id=" + pID;
                (flagOpt == "1" || flagOpt == '') ? $.colorbox({ width: "95%", height: "85%", iframe: true, href: todoPageUrl, onClosed: function () { } }) :  $.colorbox({ width: "95%", height: "85%", iframe: true, href: todoPageUrl, onClosed: function () { } });
                
            }
            if (view == 'followup') {
                $.colorbox({
                    fixed: true, width: "90%", height: "85%", iframe: true, href: "Followup.aspx?mode=add&id=0",
                    onClosed: function () {
                        __doPostBack('<%= drpFollowup.UniqueID %>', '')
                    }
                });
            }
            if (view == 'leave') {
                ...
                success: function (response) {
                    ...
                    for (i = 0; i < markers.length; i++) {
                        ...
                        if (data.flag == 0) {
                            ...
                            else if (ctrlDiv == 'dash_GraphicalClinic') {
                                $('#' + ctrlDiv).css('display', 'flex');
                            }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="pageheader texturePaper2" style="padding-bottom:0px !important; font-family:'Montserrat',sans-serif;">
        <h1 class="pagetitle contenttitle2" style="display: block !important;">&nbsp;&nbsp;
            <asp:DropDownList ID="drpDashboard" CssClass="form-control dropdown float-left" runat="server" ClientIDMode="Static" style="width:250px; background-color:navy; color:white; margin-left:10px;" AutoPostBack="true" OnSelectedIndexChanged="drpDashboard_SelectedIndexChanged" Visible="true">
    ...
    <div id="contentwrapper" class="contentwrapper" style="width:99%; padding: 5px 10px !important;">
        <div class="container-fluid">
            ...
            <div id="dash_Graphical" class="row mb-1">
                ...
                <ContentTemplate>
                <uc1:myDashboardInquirySummary runat="server" ID="myDashboardInquirySummary" ClientIDMode="Static" />                
                </ContentTemplate>
                </asp:UpdatePanel>--%>
            ...
            <%--This section Define "Graphical Analysis for FEMICURE GRAPHICAL ANALYSIS" --%>
            <div id="dash_GraphicalClinic" class="row mb-1">
                <%--<uc1:myDashboardInquiryClinicSummary runat="server" ID="myDashboardInquiryClinicSummary" ClientIDMode="Static" />--%>
                <div class="col m6 pr-0">
                    <!-- solid sales graph -->
                    <div class="card bg-info-gradient" style="background: #ced3d4; background: -webkit-gradient(linear,left bottom,left top,color-stop(0,#ced3d4),color-stop(1,#89d8e4));">
                        <div class="card-header no-border" style="padding: 5px 10px;">
                            <h5 class="card-title"><i class="fa fa-area-chart ml-2 mr-2"></i><span id="spAnalysis11">Annual Statistics</span></h5>
                        </div>
                        <div class="card-body" style="padding: 0px !important;">
                            <div class="chart" id="linechartclinic" runat="server" clientidmode="static" style="position: relative; height: 200px;"></div>
                        </div>
                        <!-- /.card-body -->
                    </div>
                </div>
                <div class="col m6">
                    ...
                    <div class="card bg-info-gradient" style="width: 49%; display: inline-table; background: #17a2b8; background: -webkit-gradient(linear,left bottom,left top,color-stop(0,#17a2b8),color-stop(1,#89d8e4));">
                        <div class="card-header no-border" style="padding: 5px 10px;">
                            <h5><i class="fa fa-pie-chart ml-2 mr-2"></i><span id="spAnalysis22">City</span></h5>
                        </div>
                        <div class="card-body" style="padding: 0px !important;">
                            <div class="chart tab-pane" id="donutchart1clinic" runat="server" clientidmode="static" style="position: relative; height: 205px;"></div>
                        </div>
                        <!-- /.card-body -->
                    </div>
                    <!-- Donut : Product Group -->
                    <div class="card bg-info-gradient" style="width: 49%; display: inline-flex; background: #17a2b8; background: -webkit-gradient(linear,left bottom,left top,color-stop(0,#17a2b8),color-stop(1,#89d8e4));">
                        <div class="card-header no-border" style="padding: 5px 10px;">
                            <h5><i class="fa fa-pie-chart ml-2 mr-2"></i><span id="spAnalysis33">Speciality</span></h5>
                        </div>
                        <div class="card-body" style="padding: 0px !important;">
                            <div class="chart tab-pane" id="donutchart2clinic" runat="server" clientidmode="static" style="position: relative; height: 205px;"></div>
                        </div>
                        <!-- /.card-body -->
                    </div>
                </div>
                ...
                <div id="divWidget">
                    ...
                    <div id="dash_FollowUp" data-listing-price="1" class="card m-1" style="width:49%; box-shadow: -6px 8px 6px -6px black;">
                        <div class="card-header p-2 textureBubble">
                            ...
                            <asp:DropDownList ID="drpEmployee" CssClass="lightDropdown float-right" runat="server" ClientIDMode="Static" AutoPostBack="true" OnSelectedIndexChanged="drpEmployee_SelectedIndexChanged"></asp:DropDownList>
                            </h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body" style="min-height:250px; max-height:250px; overflow-y:auto;">
                    ...
                    <div id="dash_SalesOrder" data-listing-price="2" class="card m-1" style="width:100%; box-shadow: -6px 8px 6px -6px black;">
                     ...
                     <!-- Sales Order List by billing status-->
                    <div id="dash_OrderBillStatus" data-listing-price="2" class="card m-1" style="width:100%; box-shadow: -6px 8px 6px -6px black;">
                        <div class="card-header p-2 textureBubble">
                            <h3 class="card-title" style="margin-bottom:0px; font-size: 1.25rem; font-weight: 400;"><i class="fa fa-certificate ml-2 mr-2"></i>Proforma Invoice List By Billing Status
                            <asp:DropDownList ID="drpBillStatus" runat="server" ClientIDMode="Static" CssClass="lightDropdown float-right" AutoPostBack="true" OnSelectedIndexChanged="drpBillStatus_SelectedIndexChanged">
                                <asp:ListItem Text="Pending" Value="Pending" />
                    ...
                    <!-- Inquiry List by status-->
                    <div id="dash_InquiryStatus" data-listing-price="2" class="card m-1" style="width:100%; box-shadow: -6px 8px 6px -6px black;">
                        <div class="card-header p-2 textureBubble">
                            <h3 class="card-title" style="margin-bottom:0px; font-size: 1.25rem; font-weight: 400;"><i class="fa fa-certificate ml-2 mr-2"></i>Inquiry List By Status
                            <asp:DropDownList ID="drpInquiryStatus" runat="server" ClientIDMode="Static" CssClass="lightDropdown float-right" AutoPostBack="true" OnSelectedIndexChanged="drpInquiryStatus_SelectedIndexChanged">
                                <asp:ListItem Text="Pending" Value="Pending" />
                                <asp:ListItem Text="Quotation Generated" Value="QuotationGenerated" />
                                <asp:ListItem Text="Sales Order Generated" Value="SalesOrderGenerated" />
                                <asp:ListItem Text="Sales Bill Generated" Value="SalesBillGenerated" />
                            </asp:DropDownList>
                            </h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body w-100" style="min-height:250px; max-height:250px; overflow-y:auto;">
                            <uc1:MyInquiryStatus runat="server" ID="MyInquiryStatus" />
                        </div>
                    </div>
                    ...
                    <!-- Pending Medical Lead List -->
                    <div id="dash_MedicalLead" data-listing-price="2" class="card m-1" style="width:100%; box-shadow: -6px 8px 6px -6px black;">
                        <div class="card-header p-2 textureBubble">
                            <h3 class="card-title" style="margin-bottom:0px; font-size: 1.25rem; font-weight: 400;"><i class="fa fa-certificate ml-2 mr-2"></i>Pending Bill List
                            <asp:DropDownList ID="drpPendingMedicalLead" runat="server" ClientIDMode="Static" CssClass="lightDropdown float-right" AutoPostBack="true" OnSelectedIndexChanged="drpPendingMedicalLead_SelectedIndexChanged">
                                <asp:ListItem Text="Pending" Value="Pending" />
                        ...
                        <!-- /.card-header -->
                        <div class="card-body w-100" style="min-height:250px; max-height:250px; overflow-y:auto;">
                            <uc1:myPendingMedicalLead runat="server" id="myPendingMedicalLead" />
                        </div>
                    </div>
    ...
    <%--    <div class="rightfixed">
	        <a onclick="_ct('bflrhs', 'dtpg');" href="images/SaveData.png" class="frilst dladv">Facebook</a> 
	        <a onclick="_ct('ccrhs', 'dtpg');" href="images/SaveData.png" class="cscare dladv">Twitter</a> 
		    <a onclick="_ct('jdscl', 'dtpg');" href="images/SaveData.png" class="jdscl dljdscl">Outlook</a>
	    </div>--%>
    <script type="text/javascript">
...
<%--    <asp:Timer ID="EventTimer" runat="server" Interval="10000" Enabled="false" OnTick="EventTimer_OnTick" ClientIDMode="Static"/>--%>
</asp:Content>
